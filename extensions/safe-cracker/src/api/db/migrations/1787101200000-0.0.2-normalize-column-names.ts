...
export async function up(dataSource: DataSource): Promise<void> {
    for (const [table, columns] of Object.entries(COLUMN_RENAMES)) {
        for (const [oldName, newName] of Object.entries(columns)) {
            const existingColumns: Array<{ column_name: string }> = await dataSource.query(
                `SELECT column_name
            ...
            if (names.has(oldName) && !names.has(newName)) {
                const identifiers = [SCHEMA, table, oldName, newName].map(
                    (identifier) => `"${identifier.replaceAll('"', '""')}"`,
                );
                await dataSource.query(
                    `ALTER TABLE ${identifiers[0]}.${identifiers[1]} RENAME COLUMN ${identifiers[2]} TO ${identifiers[3]}`,
                );
            }
        }
    }
