...
let workflowRuntimeDeviceControlExecutor: DeviceControlExecutorHandler | undefined;

export const registerDeviceControlExecutor = (executor: DeviceControlExecutorHandler): void => {
    workflowRuntimeDeviceControlExecutor = executor;
};

export class DeviceControlExecutor implements INodeExecutor {
    public readonly type = "device_control" as FlowGramNode;

    public async execute(context: ExecutionContext): Promise<ExecutionResult> {
        if (!workflowRuntimeDeviceControlExecutor) {
            throw new Error("DeviceControl executor is not registered");
        }
        const outputs = await workflowRuntimeDeviceControlExecutor({
...
function getContext(context: ExecutionContext): DeviceControlExecutorInput["runtimeContext"] {
