...
| D16 | `MOBILE_CAMERA_ENABLED` 在 **PR 1** 引入。唯一判定：`isMobileCameraEnabled() === (process.env.MOBILE_CAMERA_ENABLED === "true")`。编辑器经 `GET /api/mobile/config` 读取，**禁止** `import.meta.env.VITE_*`。PR 8 只写操作手册 | Vite 构建期 env 与运行时开关会脱节；`"0"` / `"FALSE"` 不得当成开启 |
...
开关 `MOBILE_CAMERA_ENABLED=false` 时，即使误发 `camera.session.start`，已发版且含本功能的 App 仍应处理（服务端不应下发）。**未含相机代码的旧 App**：对未知 `type` 只回 `UNSUPPORTED_MESSAGE`、保持连接。真正的保护是服务端开关默认关。
...
开关判定（唯一函数，warmup / executor / captures / `GET config` 共用）：
...
export class MobileController {
  @Get("config")
  async config() {
    return { cameraEnabled: isMobileCameraEnabled() };
  }
...
仅 `projectType === "application"` 且服务端开关打开时出现在面板。前端 **GET `/api/mobile/config`**（AuthGuard，**不需要** `X-Installation-Id`）读 `{ cameraEnabled: boolean }`，由 React Query / 编辑器 context 缓存。**禁止** `import.meta.env.VITE_MOBILE_CAMERA_ENABLED`：那是构建期常量，无法在不重编前端的情况下关掉生产节点。
...
`cameraEnabled === isMobileCameraEnabled()`。编辑器节点面板、iOS 是否展示相机相关 UI，都读这个，不读 Vite env。
